                ...
                html.Div(className = "four-columns-div-user-controls", 
                         children = [
                            html.Div([daq.PowerButton(id='my-toggle-switch',
                                                     label= {'label':'Connect OPC', 'style' : {'fontSize' : '22px', 'fontWeight' : "bold"}},
...
def opcLoadingData (on):
    ...
    if on == 1:
        visibilty  = {'width': '100%',
                      ...
                      'margin': '20px',
                      'visibility' : 'visible'}
        data_list = ['CoAd', 'ComManCoP2', 'ComManCoP3P4P5', 'ComManPompeSec', 'CompteurEnergie', 'CoP2', 'CtempDepChauff', 
                     'D1', 'D2', 'D3', 'D4', 'MarcheBruleur', 'Teg', 'SdeBasBouMelange', 'SdeBasHauMelange', 'TambN3', 'Tb1', 
                                ...
                                [
                                     ...
                                     dbc.Modal(
                                        [
                                            ...
                                            dbc.ModalBody("Vous pouvez choisir maximum 20 valeur"),
                                            dbc.ModalFooter(
                                                        ...
                                                        children=[
                                                            html.Div(
                                                                     dcc.Dropdown(id='dropdownRight', 
                                                                            options=[{'label': i, 'value': i} for i in extra_data_list],
                                                                            multi=False, 
                                                                            value = '',
                                                                            style={ "cursor": "pointer"},
                                                                            className='stockSelectorClass',
...
def edit_list2(ncr1,ncr2, valeur, children):
    ...
    if triggered_buttons == "showRight":
        ...
        new_listRight = html.Div([html.Div([  
            html.Div([dcc.Markdown('''*{}'''.format(valeur), id="checklistValeur",style = {'height' : '1rem', 'fontFamily' : 'arial','color' : 'black', 'fontSize' : '1.2rem'}),
                      ...
                      html.P(mesure1(valeur),style = {'margin':'0.1rem 0','color' : 'black','height' : '2rem', 'fontFamily' : 'arial','fontSize' : '1.2rem',}),
    ...
    if on == 1:
        ...
        loadTab1 = html.Div([html.Div([html.Div([html.Div([dcc.Dropdown(id='firstChoosenValue',
                                ...
                                html.Div([dbc.Input(id='operation',
                                                    ...
                                                    min=-10000, max=10000, step=1, 
                                                    bs_size="sm",
                                                    style = {'width': '10rem',"marginTop" : "2rem",'height' : '5rem', 'textAlign' : 'center'},
                                                    autoFocus=True,
                                                    placeholder = "result"),
                                         dbc.Input(id='intersection',
                                                    ...
                                                    min=-10000, max=10000, step=1, 
                                                    bs_size="sm",
                                                    style = {'width': '10rem',"marginTop" : "2rem",'height' : '2rem', 'textAlign' : 'center'},
                                                    autoFocus=True,
                                                    placeholder = "Intersection")],className = 'aa')],className ="ab"),
...
def res2(val,radiograph,firstshape, secondshape,sliderheight,sliderwidth,
    ...
    if len(retrieve)> 0 :
        ...
        if 'date' not in df.columns:
            ...
            dt = ["{}-{:02.0f}-{:02.0f}_{:02.0f}:{:02.0f}:{:02.0f}".format(d.year, d.month, d.day,d.hour,d.minute,d.second) for d in df_shape.index]
        print('baseval',dt[0])
        fig = go.Figure()
        for i_val in range(len(val)) :
            ...
            if len(firstshape)==2 and len(secondshape) == 2:
                ...
                print('aaaa',c)
                print('aaaa',d)
                if len(set(range(a,b)).intersection(set(range(c,d))))>= 1 or len(set(range(c,d)).intersection(set(range(a,b))))>= 1:
                    if a <= c :
                        if len(differance) ==2:
                            differance.pop(0)
                            differance.append(b)
...
exportdatalist = [None,None,['Date', 'Firstchoosenvalue', 'leftfirstIntegralVal','leftSecondIntegralVal','leftIntegral',
                             'Secondchoosenvalue','rightFirstIntegralVal','rightSecondIntegralVal','rightIntegral','result','Interection']]
